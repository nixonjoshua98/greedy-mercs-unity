...
class UserItemQueryContainer(DatabaseQueryContainer):
    ...
    async def update_items(self, uid: Union[str, ObjectId], update: dict, *, upsert: bool = True) -> bool:
        return (await self.client.get_default_database()["userItems"].update_one(
            {"userId": uid}, await _before_update(self, uid, update), upsert=upsert
        )).modified_count > 0
    ...
    async def update_and_get(self, uid: Union[str, ObjectId], update: dict) -> dict:
        return await _after_find(
            await self.client.get_default_database()["userItems"].find_one_and_update(
                {"userId": uid}, await _before_update(self, uid, update),
...
async def _move_inc_to_set(items: UserItemQueryContainer, uid: Union[str, ObjectId], update: dict, key: str):
    """
    ...
    if inc_amount := update["$inc"].get(key):
        amount = await items.get_item(uid, key)
